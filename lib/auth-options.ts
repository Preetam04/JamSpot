  ...
  providers: [
    ...
    Credentials({
      ...
      async authorize(credentials) {
        ...
        const passworValidation = passwordSchema.safeParse(
          credentials.password
        );

        if (!passworValidation.success) {
          throw new Error(passworValidation.error.issues[0].message);
        }
